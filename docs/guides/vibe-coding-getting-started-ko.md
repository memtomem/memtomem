...
```bash
uvx --from 'memtomem[onnx]==0.6.5' mm init --preset minimal --non-interactive --mcp skip
uvx --from 'memtomem[onnx]==0.6.5' mm status
```
...
`memtomem[onnx]==0.6.5`입니다. 활성화 확인과 데이터 조회 성공은 서로 다른 단계입니다.
...
```bash
uvx --from 'memtomem[onnx]==0.6.5' mm mem init --scope project_local
```
