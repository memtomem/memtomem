...
| `held_source` | warn | A missing source's chunks are retained but hidden from ordinary search while availability is uncertain. | Restore the source or review with `mm gc orphan-sources`; purge with `--apply` after confirming deletion. |
...
| `ambiguous_index_line` | warn | The line names something the doctor won't resolve on a guess: a link target that isn't a plain relative path — one carrying a query (`live.md?view=1`), a percent-escape (`live%2Emd`), a scheme (`urn:live.md`) or a space (`<x y.md>`) — link syntax left unclosed (`- [B](b.md`), or a *contested wikilink label*: a genuine `[[memo]](note)` and an escaped same-name pointer (`[\[memo\]](file.md)`) share the line, both decode to the label `[memo]`, and the doctor cannot tell which occurrence is the wikilink, so neither is trusted. Such links are neither link-checked nor counted as listed, so the file one may have meant can also surface as an `index_orphan`. Ordinary filenames needing markdown escaping (`notes_(v2).md`, `notes_\(v2.md`, `notes_&amp;v2.md`) are *not* flagged — they are read as Markdown and resolve normally. | Rewrite the target as a plain relative filename; for a contested wikilink label, rename the wholly bracketed label if the link is meant as a pointer (un-escaping it makes the occurrence a wikilink instead). Reported as warn, not error. `--fix` will not touch such a line: an unreadable link is never *provably* dead, so a line holding only these is not a `--fix` candidate at all, and one sitting beside a genuinely dead link makes that line a [skip](#fixing-broken-links) — the rest of the file is still fixed. |
...
stays hidden. Repair the file and run `mm index <file>`. If the source is no
longer wanted, remove it first, then review and apply `mm gc orphan-sources`.
...
> **Read-only by default.** Without `--fix`, `mm memory doctor` reports and never writes. Apply the per-check remediation above — most commonly `mm index <dir>` to close a coverage gap. Reserve `--force` for embedding changes, suspected index corruption, or scope re-resolution after configuration changes because it re-embeds unchanged chunks. It does **not** re-resolve namespaces — each file keeps the namespace its chunks are stored under; applying changed namespace rules to already-indexed files is `mm index --reassign-namespaces` ([ADR-0033](../../adr/0033-force-reembed-vs-namespace-reassignment.md)). Plain indexing already detects heading-only changes and refreshes their retrieval metadata and embeddings.
...
- **A skip is never success.** `--fix` exits `1` whenever it left a dead pointer behind (in dry-run too), could not read an index file, or refused a directory under `indexing.read_only_memory_dirs` (repairing an index means rewriting it), and `0` when there was nothing to do or every candidate was handled. `--fix --json` keeps the outcomes apart in `status`: `clean` (no dead pointers), `would-fix` / `fixed` (all candidates handled), `would-partial` / `partial` (some candidate skipped), and `error` (an index file could not be read — decode or I/O failure, in either the analysis read or the locked `--apply` re-read). `error` outranks the other statuses whenever any file hit it: a skip is a complete account of what remains, an unread file is no account at all. Each file object carries `removed` and `skipped` (`{line, text, reason}`) lists plus an `error` message (`null` when the file was read), and `summary` counts `{files, lines, skipped, errors, read_only_refused}` — so "no dead pointers" never reads the same as "dead pointers this tool won't touch," or as "a file this run never opened." A refused read-only root is listed by path in a top-level `read_only_refused` array and ranks with `skipped` in `status` (`would-partial` / `partial`): its dead pointers are still on disk, so the run is an incomplete account of what was asked for.
