...
| `ambiguous_index_line` | warn | A link looks broken, but sits on a line the parser can't read unambiguously — a code span, a stray bracket, or a parenthesis inside the target (`[Live](notes_(v2).md)`) — so the "broken" verdict may be a mis-read rather than a real dead pointer. | Check the line by hand: either the target really is dead (fix it), or the line's markdown needs simplifying. Reported as warn, not error, and `--fix` refuses to touch these lines. |
