...
| `ambiguous_index_line` | warn | The line carries link syntax the parser can't read unambiguously: a parenthesis inside the target (`[Live](notes_(v2).md)`), an escaped (`notes_\(v2.md`) or angle-bracketed (`<x y.md>`) target, a link inside a code span, or link syntax it couldn't close. Its links are neither link-checked nor counted as listed, so the file it points at may also surface as an `index_orphan`. | Simplify the line's markdown (rename the target to drop the parens, or move the quoted link out of the hook) to bring it back under the checks. Reported as warn, not error, and `--fix` refuses to touch these lines. |
