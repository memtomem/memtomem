...
| `ambiguous_index_line` | warn | The line's links can't be resolved as written, so the doctor won't guess: a target that isn't a plain relative path — a parenthesis (`[Live](notes_(v2).md)`), an escape (`notes_\(v2.md`), a character reference (`notes_&amp;v2.md`) or the angle form (`<x y.md>`) all make the literal text differ from what Markdown resolves — or a link quoted inside a code span, or link syntax left unclosed. Such links are neither link-checked nor counted as listed, so the file one points at may also surface as an `index_orphan`. | Simplify the line: rename the target to a plain relative filename, or move the quoted link out of the hook. Reported as warn, not error, and `--fix` refuses to touch these lines. |
