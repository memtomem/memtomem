...
| `dangling_wikilink` | info | A `[[name]]` on an index line with no `name.md` in the memory root (`[[name\|alias]]` resolves on `name`). Either a *forward reference* — a memory worth writing later, which the [memory conventions](#dangling-wikilinks) allow — or a stale link to a deleted memo. The doctor can't tell the two apart, so this never fails a run, and `--fix` never acts on it. | Write the memo, fix the name, or leave it as a deliberate forward reference. |
