...
- **A skip is never success.** `--fix` exits `1` whenever it left a dead pointer behind (in dry-run too), and `0` when there was nothing to do or every candidate was handled. `--fix --json` keeps the same three outcomes apart in `status`: `clean` (no dead pointers), `would-fix` / `fixed` (all candidates handled), and `would-partial` / `partial` (some candidate skipped). Each file object carries `removed` and `skipped` (`{line, text, reason}`) lists, and `summary` counts `{files, lines, skipped}` — so "no dead pointers" never reads the same as "dead pointers this tool won't touch."
