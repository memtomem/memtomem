...
| `stale_index` | warn | An indexed file has moved past its chunks, so `mem_search` answers from the old index. Detected by re-chunking the file and diffing against the DB on content hash, heading hierarchy **and line range** — so a real edit is caught, a reordering of unchanged sections is caught (it changes which neighbours a context-window search returns), and a bare `touch` or an identical re-save is not reported. Not a timestamp comparison: chunk `updated_at` advances on metadata-only writes and stays put through a no-op re-index, so it would both hide real drift and flag untouched content. Two gaps, both inherited from `mm index`, so the check stays silent where a plain re-index would write nothing: collapsing two byte-identical chunks into one ([#2123](https://github.com/memtomem/memtomem/issues/2123) — the orphan rows survive even `--force`), and editing a section's `> tags: [...]` blockquote ([#2124](https://github.com/memtomem/memtomem/issues/2124) — the DB keeps the old tags that `mem_search(tag_filter=…)` reads; `mm index --force <file>` does clear this one). | `mm index <file>` |
| `stale_index_blocked` | warn | The same drift on a file whose new content matches a redaction pattern. `mm index` **skips** such a file rather than failing it, so its chunks stay stale indefinitely and `mm index <file>` alone will not clear it. Hit counts only — matched text is never printed. Items name the path relative to the memory root printed above them (`mm index` itself resolves a relative path against your current directory, so join the two or `cd` to the root first). | Remove or move the matched text, then `mm index <file>`; or, once you have triaged the hit as a false positive, `mm index --force-unsafe <file>` (audit-logged, and hard-refused for `project_shared` files). |
