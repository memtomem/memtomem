...
- **Re-index from scratch (destructive, recommended when you really are
  ...
  the namespace its chunks are stored under. The one thing that still
  overrides that is a namespace the caller *names* — `--namespace` on the CLI,
  `mem_index(namespace="...")` over MCP — which is intent, and moves the rows
  it reaches.

  > **A namespace you did not type does not move anything.** An agent session
  > (`mem_session_start(agent_id=...)`) or a current namespace set with
  > `mem_ns_set` binds *new* files only: a source the index has never seen is
  > stamped with it, and an already-indexed file keeps what it has. So the
  > recovery is safe to run from either surface.
...
| `MEMTOMEM_INDEXING__EXTRACT_ENTITIES` | `true` | Extract entities (person, date, decision, action_item, technology, concept) for every chunk the indexer writes, populating `chunk_entities`. Regex only — stdlib patterns, no model and no network, so it costs microseconds next to the embedding call on the same write. The LLM extractor stays opt-in via `mem_entity_scan(overwrite=True)`. Governs every write path that stores new content — the indexer, bundle imports, and consolidation summaries. Turning this off leaves the table at whatever a scan last wrote |
...
class matching the single character `a` or `"` — is taken as JSON. To store
such a pattern verbatim, wrap it in a JSON array of its own:
`'["[\"a\"]"]'`.
...
| `codex` | `~/.codex/memories/` | Codex memories ([official docs](https://learn.chatgpt.com/docs/customization/memories)) |
...
the chunk's content is re-indexed. Bundle imports and consolidation summaries —
content that never passes through the indexer — extract on their own write
paths, under the same knob. An import writes entities only for the chunks it
genuinely adds: a record whose content the store already has keeps the entities
it has, which may have come from a richer `mem_entity_scan` LLM pass — so coverage tracks the corpus instead of
...
When enabled, all policies created via `mem_policy_add` are executed periodically. Policies can always be run on demand via `mem_policy_run` regardless of this setting. The action count semantics vary by policy type (e.g. archived chunks vs consolidated groups). Each applied run is recorded with `source=scheduler`, so scheduled runs can be audited after the fact via `mem_policy_list(runs=N)` — see [automation](reference/automation.md).
...
| Variable | Default | Description |
|----------|---------|-------------|
| `MEMTOMEM_SESSION_SUMMARY__AUTO` | `true` | When `mem_session_end` is called without `summary=`, run an LLM auto-summary over chunks the session recorded as written |
| `MEMTOMEM_SESSION_SUMMARY__MIN_CHUNKS` | `5` | Minimum recorded write chunks before auto-summary fires |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `MEMTOMEM_TOOL_MODE` | `core` | Which MCP names are exposed: `core` (9), `standard` (38 incl. `mem_do`), `full` (100 current tools + deprecated `mem_context_migrate` alias) |
