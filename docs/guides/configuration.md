...
| Antigravity CLI | Successor to Gemini CLI ([transition notice](https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/)). Loads global instructional rules from `~/.gemini/GEMINI.md` — the same hardcoded path the legacy Gemini CLI used — plus repo-root `AGENTS.md` and `.agents/` skills ([docs](https://antigravity.google/docs/agent-features)). | Not added as a watched `memory_dirs` provider because the canonical memory surface is a file, not a directory. Use `mm ingest gemini-memory` for a one-shot import with `gemini-memory:<slug>` namespaces — the command keeps the `gemini-memory` name because Antigravity's global memory file is still `~/.gemini/GEMINI.md`. |
