...
> arguments** as the plugin — `uvx --from memtomem==0.3.14 memtomem-server`
...
>   environment, or `uvx --isolated --from "memtomem[all]==0.3.14"
...
"memtomem[all]==0.3.14" memtomem-server`. If an older user-scope registration
...
```bash
uv tool install 'memtomem==0.3.14'
```
