...
> arguments** as the plugin — `uvx --from 'memtomem[onnx]==0.6.3' memtomem-server`
...
>   environment, or `uvx --isolated --from "memtomem[all]==0.6.3"
...
"memtomem[all]==0.6.3" memtomem-server`. If an older user-scope registration
...
```bash
uv tool install 'memtomem==0.6.3'
```
