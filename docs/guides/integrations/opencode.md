...
exact-pinned memtomem MCP server, seven slash commands, and three read-only
...
The npm release for this core version is `opencode-memtomem@0.3.7`, bundling
core `0.6.5`. Add it through OpenCode's singular `plugin` configuration key
(there is no `opencode plugin add` command):

```json
{"plugin": ["opencode-memtomem@0.3.7"]}
```
...
```json
{
  ...
  "mcp": {
    "memtomem": {
      "type": "local",
      "command": ["uvx", "--isolated", "--from", "memtomem[all]==0.6.5", "memtomem-server"],
      "enabled": true,
      "timeout": 60000,
...
|---|---|
| MCP | Exact-pinned `memtomem[onnx]==0.6.5`, core tool mode (plugin); `[all]` no-install runtime (manual MCP) |
| Commands | `memtomem-search`, `recall`, `status`, `remember`, `index`, `setup`, `handoff` |
