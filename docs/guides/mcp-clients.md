...
--isolated --from "memtomem[all]==0.6.4" memtomem-server`.
...
command (`uvx --from 'memtomem[onnx]==0.6.4' memtomem-server`), so installing the
...
The npm plugin for this core version is `opencode-memtomem@0.3.6` (bundling
core `0.6.4`). Add it through OpenCode's plugin form —
`{"plugin": ["opencode-memtomem@0.3.6"]}` in `opencode.json`. The plugin
...
> Running `uvx --isolated --from "memtomem[all]==0.6.4" memtomem-server`
