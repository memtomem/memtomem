...
`0.3.13`). Add it through OpenCode's plugin form —
`{"plugin": ["opencode-memtomem@0.1.3"]}` in `opencode.json`. The plugin
supplies an exact-pinned MCP server, six commands, three read-only skills,
and conservative permissions.
