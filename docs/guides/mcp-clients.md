...
--isolated --from "memtomem[all]==0.6.2" memtomem-server`.
...
command (`uvx --from 'memtomem[onnx]==0.6.2' memtomem-server`), so installing the
...
The npm plugin for this core version is `opencode-memtomem@0.3.4` (bundling
core `0.6.2`). Add it through OpenCode's plugin form —
`{"plugin": ["opencode-memtomem@0.3.4"]}` in `opencode.json`. The plugin
...
> Running `uvx --isolated --from "memtomem[all]==0.6.2" memtomem-server`
