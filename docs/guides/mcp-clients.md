...
--isolated --from "memtomem[all]==0.6.0" memtomem-server`.
...
command (`uvx --from memtomem==0.6.0 memtomem-server`), so installing the
...
The npm plugin for this core version is `opencode-memtomem@0.3.1` (bundling
core `0.6.0`). Add it through OpenCode's plugin form —
`{"plugin": ["opencode-memtomem@0.3.1"]}` in `opencode.json`. The plugin
...
> Running `uvx --isolated --from "memtomem[all]==0.6.0" memtomem-server`
