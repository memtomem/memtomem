...
| 0021 §"Open questions" §2 | Atomic backend `POST /api/context/sync-all` (lock-free per-type cores under one outer lock) | Need for cross-type all-or-nothing sync OR a partial-failure report from the sequential front-end Sync All (criteria in ADR §"Open questions") | (none — tracked in ADR) | (event-driven) |
