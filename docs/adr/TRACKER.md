...
| 0034 §"Deferral" | Session-touched entities as a derived view over write provenance × `chunk_entities` (D1–D3) | Precondition lands: extraction runs at index time for every chunk by default and re-index re-extracts (criteria in ADR-0034 §"Deferral"). Index-time extraction shipped in #2145; still open is backfill for stores indexed before it | [#2133](https://github.com/memtomem/memtomem/issues/2133) (tracker) · [#2145](https://github.com/memtomem/memtomem/issues/2145) (precondition) | (event-driven) |
