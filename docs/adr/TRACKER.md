...
|-----|-------------------|--------------------|----------------|-------------|
| 0035 §"Deferred, with the name kept compatible" | Converting `mm doctor` from a leaf command into a `click.Group` with axis subcommands (`mm doctor runtime` / `mm doctor store`), and/or rolling the other three doctors' findings up into it | A second runtime-axis consumer of the shared status vocabulary appears (making the result-model extraction concrete) OR users report confusion about which doctor to run (criteria in ADR-0035) | [#2226](https://github.com/memtomem/memtomem/issues/2226) | (event-driven) |
| 0035 §"Deferred, with the name kept compatible" | Acting on the accumulation `mm doctor` reports — an opt-in terminate, or a server self-exiting when its parent is gone | A decision on process lifecycle ownership; reaping an STM proxy's children belongs to the STM repo, which this one may not depend on (criteria in ADR-0035 and #2226 options 3/4) | [#2226](https://github.com/memtomem/memtomem/issues/2226) | (event-driven) |
| 0036 §"Maintenance and browse surfaces stay whole-store" | An optional scope filter on the operator-tier corpora (`mem_dedup_scan`, `mem_export`, consolidation candidates, web `GET /chunks?source=` and `GET /sources`), which today read the whole store and emit full ids and content previews | A user wants project-scoped maintenance (dedup or export limited to the current project) OR a second surface is found emitting foreign-project content by default (criteria in ADR-0036 §"What this guarantee is, and is not") | [#2238](https://github.com/memtomem/memtomem/issues/2238) | (event-driven) |
...
| 0030 §10 "API compatibility" | Labeled skill fan-out — Push a *non-`latest`* skill label to runtimes (needs a `label` argument the skill generators lack), and with it the user-facing `create`/`promote`/`delete-label` version ops for skills, all refused in campaign 1 | PR-G4 lands (skills gain real tree snapshots, so a label finally has something to point at) OR a concrete request to roll out/roll back a specific skill version (criteria in ADR-0030 §10 "API compatibility") | (none — tracked in ADR) | (event-driven) |
...
| 0006 §"Axis G" | Opt-in, default-off `export_dir` allowlist (or a suspicious-destination pre-write gate) for the MCP `mem_export` / `mem_import` filesystem paths — the *true* pre-write control deferred behind G.1's accept-as-residual | Risk-appetite shift OR memtomem blesses unauthenticated non-local MCP exposure as a supported / default posture (criteria in ADR-0006 §"Axis G" reopen trigger; cross-ref ADR-0029 / #1485) | (none — tracked in ADR) | (event-driven) |
| 0037 §"Considered & rejected" | A `--force-unsafe` egress valve on `mm context export` for `user` / `project_local` sources — today a secret refuses the export from every tier with no bypass | A concrete report of the own-machine case: a user shipping their own artifact to their own second machine and blocked by a secret they accept. The question then is whether "the same person on another machine" is a distinct destination class, not whether the valve should exist for handing files to other people (criteria in ADR-0037 §"Considered & rejected"; the gate itself is §4) | [#2298](https://github.com/memtomem/memtomem/issues/2298) | (event-driven) |
| 0037 §"Consequences" | Carrying non-ASCII artifact filenames in a bundle — v1's path grammar is an ASCII allowlist, so they are refused loudly | A report of an artifact that cannot be shared because of it. What has to be settled then is a Unicode normalization and collision policy, not a longer denylist (criteria in ADR-0037 §2 and §"Consequences") | [#2298](https://github.com/memtomem/memtomem/issues/2298) | (event-driven) |
| 0034 §"Deferral" | Session-touched entities as a derived view over write provenance × `chunk_entities` (D1–D3) | Precondition lands: extraction runs at index time for every chunk by default and re-index re-extracts (criteria in ADR-0034 §"Deferral"). Fully met: index-time extraction shipped in #2145, and stores indexed before it are backfilled automatically at startup — D1–D3 are unblocked (the derived view must gate on the `entity_backfill_v1` marker reading `done`) | [#2133](https://github.com/memtomem/memtomem/issues/2133) (tracker) · [#2145](https://github.com/memtomem/memtomem/issues/2145) (precondition) | (event-driven) |
