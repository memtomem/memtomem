...
| 0022 §"Open questions" §1 | Skills versioning — directory-tree snapshot model (`versions/v1/<SKILL.md + assets>`) with the same label layer (v1 covers agents + commands only) | **Answered**: the snapshot model is now decided in ADR-0030 §10 (amended 2026-07-19) and implemented by PR-G0..G4; strike this row when G4 lands | (none — tracked in ADR) | (event-driven) |
