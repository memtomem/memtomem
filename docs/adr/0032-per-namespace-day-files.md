...
> surfaces a *user* drives. `force` now preserves for every caller that does
> not *name* a namespace — an explicit one still wins, while a namespace
> `mem_index` inherits from session context binds only files with no stored
> rows (#2104) — and rule re-application moved to an explicit
> `--reassign-namespaces`. The rest of
