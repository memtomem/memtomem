...
> The plugin launches `memtomem[onnx]==0.6.3`. A manual registration whose launch
...
```bash
claude mcp add memtomem -s user -- uvx --isolated --from "memtomem[all]==0.6.3" memtomem-server
```
