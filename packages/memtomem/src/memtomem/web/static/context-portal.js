...
if (_ctxPortalInstallGuideModal) {
    ...
    if (runtimeName === 'claude') {
      guideHtml = `
        ...
        <div class="guide-code-block">
          ...
          <pre class="guide-code"><code>claude mcp add memtomem -- uvx --isolated --from 'memtomem[all]==0.4.0' memtomem-server</code></pre>
        </div>
  ...
  "mcpServers": {
    "memtomem": {
      "type": "stdio",
      "command": "uvx",
      "args": ["--isolated", "--from", "memtomem[all]==0.4.0", "memtomem-server"]
    }
  }
}</code></pre>
        </div>
...
command = "uvx"
args = ["--isolated", "--from", "memtomem[all]==0.4.0", "memtomem-server"]</code></pre>
        </div>
      `;
