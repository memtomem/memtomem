...
async def test_mem_import_foreign_secret_rejected_then_force_unsafe(tmp_path, monkeypatch):
    ...
    storage = await _make_storage(tmp_path)
    # Session state: ``mem_import`` marks the active session's provenance
    # incomplete (#1876) — a bulk import changes the session's chunk set
    # without being summarizable from it. There is no session here, so the
    # marker is a no-op; the stub only has to model the locked handle read
    # and the ending-session seal.
    app = SimpleNamespace(
        storage=storage,
