...
```bash
uvx --from 'memtomem==0.3.14' mm init --preset minimal --non-interactive --mcp skip
uvx --from 'memtomem==0.3.14' mm status
```
...
```bash
cd /path/to/project
uvx --from 'memtomem==0.3.14' mm mem init --scope project_local
```
...
(`uvx --from memtomem==0.3.14 memtomem-server`), Claude Code suppresses the
