...
후속 작업은 별도 PR에서 아래 순서로 진행한다. 여기서 `top_k`는 최종 반환
깊이, candidate k는 BM25/dense가 RRF에 제공하는 후보 수, RRF `k`는
`1 / (k + rank)`의 순위 완화 상수, reranker pool은 재정렬 입력 수를 뜻한다.
현재 실험은 각각 `10`, `50/50`, `60`, `20`으로 고정되어 있다.

### k-sweep 단계

전체 조합을 한 번에 곱하지 않고 앞 단계의 후보를 줄인 뒤 다음 단계로 넘긴다.
모든 단계는 영어·한국어·교차언어 트랙을 따로 보고한다.

1. **RRF 상수 탐색**: reranker 없이 `top_k=10`, BM25/dense 후보 50을
   고정하고 RRF `k=[10, 30, 60, 100]`을 비교한다.
2. **반환 깊이와 후보 폭 탐색**: 1단계 상위 RRF `k`를 사용하여
   `top_k=[5, 10, 20]`, BM25/dense 후보 `[20, 50, 100]`을 비교한다.
   Recall/MRR/nDCG는 반환 깊이에 맞는 `@5`, `@10`, `@20`을 함께 기록한다.
3. **reranker pool 탐색**: 선택된 RRF `k`와 후보 폭에서 `top_k=10`을
   고정하고 pool `[10, 20, 50]`을 비교한다. pool은 항상 `top_k` 이상이어야
   하며 언어별 기본 임베딩과 BGE-M3를 각각 시험한다.
4. **반복 검증**: 탐색은 조합별 1회로 후보를 줄이고, 상위 조합은 5회,
   최종 후보와 현재 기본값은 10회 실행한다.
5. **운영 비용 측정**: 실행 간 품질·지연시간 분산, cold/warm cache,
   모델 적재 시간, peak RSS, 디스크 캐시 크기를 기록한다.

### 선택 규칙

- 현재 기본값을 항상 control로 포함하고 질의·qrel·코퍼스 해시는 고정한다.
- 영어의 macro nDCG@k와 MRR@k는 control 대비 `-0.01`보다 크게
  회귀하면 탈락시킨다.
- 한국어·교차언어 후보는 macro nDCG@k 또는 Recall@k를 개선하면서
  zero-hit을 악화시키지 않아야 한다.
- negation constraint, genre hit@1, multi-topic intent coverage 중 어느 하나도
  `-0.05`보다 크게 회귀하면 탈락시킨다.
- 품질 게이트를 통과한 후보끼리는 p95 지연시간과 메모리 사용량이 낮은
  조합을 선택한다. reranker 구성은 비-rerank 구성과 별도 품질 프로필로
  판정한다.
- 10회 확인 실행에서 같은 결론이 유지될 때만 제품 기본값 변경을 제안한다.
